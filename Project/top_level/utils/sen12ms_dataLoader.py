...
MainDataPath: Path = Path(__file__).parent.resolve().parent.parent / "data"
ClcDataPath: Path = Path(__file__).parent.resolve().parent.parent / "clc-data"
...
class SEN12MSDataset:
    ...
    def get_scene_ids(self, season):
        ...
        #     scene_list.extend([os.path.basename(s) for s in glob(os.path.join(sub_dir_path, "*"))])
    ...
    def get_patch(self, season, scene_id, patch_id, bands, convert_bounds_to_lonlat: bool = False):
        season = Seasons(season).value
        sensor = None
        ...
        with rasterio.open(patch_path) as patch:
            ...
            bounds = patch.bounds
            if convert_bounds_to_lonlat:
                crs = patch.crs
                # in_proj = pyproj.Proj(init=f"epsg:{str(crs.to_epsg())}")
                # in_proj = pyproj.CRS(f"epsg:{str(crs.to_epsg())}")
                # out_proj = pyproj.CRS("epsg:4326")
                # out_proj = pyproj.Proj(init="epsg:4326")

                left, bottom, right, top = bounds

                transformer = pyproj.Transformer.from_crs(f"EPSG:{str(crs.to_epsg())}", "EPSG:4326")
                # lons, lats = pyproj.transform(in_proj, out_proj, [left, right], [top, bottom])
                lons, lats = transformer.transform([left, right], [top, bottom])
    ...
    @classmethod
    def from_main_data_path(cls) -> "SEN12MSDataset":
        return cls(str(MainDataPath))

    @classmethod
    def from_clc_data_path(cls) -> "SEN12MSDataset":
        return cls(str(ClcDataPath))


if __name__ == "__main__":
    import time
    # Load the dataset specifying the base directory
    sen12ms = SEN12MSDataset("../../data")
